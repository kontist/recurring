...
class RecurlyData {
  ...
  all(filter, callback) {
    ...
    if (!this.enumerable) {
      log.info(`[RecurlyData/all] Not enumerable`, { options: this.options })
    ...
    return this.fetchAll(name, url, filter, (err, results) => {
      if (err) {
  ...
  fetchAll(modelName, uri, filter, callback) {
    ...
    log.info('[RecurlyData/fetchAll]', { model, iterator })
    ...
    iterators.forEachAsync(iterator, function(err, item, next) {
      ...
      log.info('[RecurlyData/fetchAll] Item', { item })

      result.push(item)
      return next()
    }, () => {
      log.info('[RecurlyData/fetchAll] Result', { result })

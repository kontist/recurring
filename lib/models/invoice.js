...
class Invoice extends RecurlyData {
  ...
  async fetchAllInvoices(filter) {
    const uri = `${Invoice.ENDPOINT}`
    let results = []

    log.info(`[Invoice/fetchAllInvoices] Start`, { uri, filter })

    try {
      results = await this.fetchAllAsync('Invoice', uri, filter)
      log.info(`[Invoice/fetchAllInvoices] End. Total results: ${results.length}`)
    }
    catch (err) {
      log.error(`[Invoice/fetchAllInvoices] Error`, err)
    }

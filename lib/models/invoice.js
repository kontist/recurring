...
class Invoice extends RecurlyData {
  ...
  fetchPDF(callback) {
    if (!this.invoice_number) {
      throw (new Error('cannot fetch a record without an invoice_number'))
    }

    log.info(`[Invoice/fetchPDF] Start`, { invoice_number: this.invoice_number })

    const uri = `${Invoice.ENDPOINT}/${this.invoice_number}`
